...
class DetailsWithPrice extends Component {
    ...
    constructor(props) {
        ...
        this.state = {
            open: false,
            quantity: 1,
            stock: this.props.item.product_variances[0].stock_count ? 'InStock' : 'Out of Stock !',
            nav3: null,
            variance_index: 0,
        }
    }
    ...
    plusQty = () => {
        if (this.props.item.product_variances[this.state.variance_index].stock_count >= this.state.quantity) {
            this.setState({ quantity: this.state.quantity + 1 })
        } else {
    ...
    changeVariance = (e) => {
        this.setState(
            {
                variance_index: Number(e.target.value),
                quantity: 1,
                stock: this.props.item.product_variances[Number(e.target.value)].stock_count ? 'InStock' : 'Out of Stock !',
            })
    }
    render() {
        const { symbol, item, addToCartClicked, BuynowClicked, addToWishlistClicked } = this.props;
        const { variance_index } = this.state;
                ...
                <div className="product-right">
                    ...
                    {item.product_variances.length > 1 ?
                        <ul >
                            ...
                            {/*    })}*/}
                            {/*</Slider>*/}
                            <select className="form-control" onChange={e => this.changeVariance(e)}>
                    ...
                    <div className="product-buttons" >
                        ...
                        <a href={`${process.env.PUBLIC_URL}/checkout`} className="btn btn-solid" onClick={() => BuynowClicked(item, this.state.quantity)} >buy now</a>
                    </div>
                    ...
                    <div className="border-product">
                        ...
                        <div className="product-icon">
                            ...
                            {/*</ul>*/}
                            <button className="wishlist-btn"><i
